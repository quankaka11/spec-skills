*Tài liệu BTC gửi (11/09) — "Spec Battle Anatomy". Nguồn gốc: `knowledge/Spec Battle Anatomy.html`. Đây là CẤU TRÚC BẮT BUỘC của spec nộp; knowledge/30 là kỹ thuật viết bên trong cấu trúc này.*

# 32 — Cấu trúc spec BTC yêu cầu (10 mục)
...
**Quy tắc chung:** nhãn tiếng Việt có dấu phải bọc trong ngoặc kép — `A["Khách đăng nhập"]`; không dùng ký tự `|`, `(`, `)` trần trong nhãn; mỗi sơ đồ ≤ 15 node (spec 3.000 từ không đủ chỗ cho hơn); **luôn kèm 1–2 câu chữ tóm tắt ngay dưới sơ đồ** — nếu BTC render lỗi hoặc Executor đọc thô, phần chữ vẫn tải được nghĩa.
