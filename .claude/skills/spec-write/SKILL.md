---
name: spec-write
description: Viết spec cho AI Executor đọc từ mô hình bài toán + RTM (dòng đã hỏi và dòng giả định) + log AI Khách hàng, theo **cấu trúc 10 mục BTC** (基本設計/詳細設計/技術仕様) có mã BR truy vết, mỗi luật gắn một mục tiêu và qua cổng khả thi F; ô không hỏi được thì điền mặc định ngành có xếp hạng rủi ro; hoặc nén spec đang có về ≤9.000 token không mất luật. Dùng 10:20–11:52 ngày thi khi người dùng nói "viết spec", "điền template", "nén spec", "spec-write", "rút gọn spec".
...
Mục tiêu: mọi dòng RTM thành một luật có mã; đủ **10 mục BTC**, không mục nào trống; mọi mục tiêu của brief có luật phục vụ; mọi luật đứng được trong thực tế; mọi ô bảng được điền; **bản nộp ≤9.000 token ước lượng** (hạn mức 10.000 — 00 §A); không bịa.

**Với hạn mức 10.000, lỗi hay gặp đổi chiều:** không phải tràn hạn mức mà là **dừng sớm** — spec 6.500 token còn ô trống ở mục 4 và 7.2 là spec chưa viết xong. Đo token lúc 11:30 và **tiêu đệm theo bảng ưu tiên knowledge/33 §5** trước khi nghĩ tới việc nộp.
...
- `${CLAUDE_PROJECT_DIR}/knowledge/30-viet-spec.md`: §1 template 10 mục chi tiết, §2 khung catch-all đặt ở mục 1.x (gồm 0.11–0.15), §3 cú pháp EARS + mẫu BR-xx, §4 checklist 17 quy tắc + 7 quy tắc nội dung (18–24), §1b luật giả định = mặc định ngành, §5 tiết kiệm token, §6 thứ tự viết, §7 cổng chất lượng 25 dòng.
...
2. **Mục 1 trước** (850 token): tên hệ thống / màn hình / chức năng, trạng thái tài liệu, chức năng làm gì — 4–5 bullet; TRONG/NGOÀI phạm vi **nguyên văn từ lượt hỏi về phạm vi** (20 §3.2 lượt 26 — dòng ⛔ trong `rtm.md`); lượt đó chưa gửi → dùng danh sách NGOÀI của domain (10) và ghi `[GIẢ ĐỊNH]`, đồng thời cảnh báo: buổi chiều mọi test của đội bị +1 điểm phạm vi (50 §6-5). Rồi **1.x catch-all** từ khung knowledge/30 §2 (12 dòng cơ bản **+ bắt buộc 0.11–0.14**; **+ 0.15 nếu spec có sơ đồ mermaid**) và **1.y glossary** nếu ngắn — chỉ thuật ngữ dễ hiểu sai, một khái niệm một tên.
3. **Mục 6 — lớn nhất (3.200 token), viết ngay sau mục 1.** Mỗi logic một mục con (danh sách gợi ý 6.1–6.9 ở knowledge/30 §1), mỗi logic đủ **ba thứ** (knowledge/33 §2):
   - (1) bảng step: ai làm gì / hệ thống phản hồi gì;
   - (2)(3) bảng `Case · Điều kiện · Kết quả mong đợi · Mã · RTM` — **bắt buộc đủ 3 loại case: bình thường, biên, lỗi**, và với ngân sách 3.200 thì **tối thiểu 5 case mỗi logic** (1 bình thường + 2 biên + 2 lỗi); mỗi bảng đủ **5 gạch**: con số · toán tử `>` hay `≥` · múi giờ · giá trị mặc định khi config trống · thứ tự ưu tiên khi nhiều case cùng đúng.
...
4. **Mục 4 Validation & message** (1.000 token — đủ chỗ cho 12–15 rule; mỗi nhánh từ chối trong mục 6 phải có một dòng message ở đây): bảng `No · FE/BE · Item · Nội dung check · Message`. **Message ghi nguyên văn trong ngoặc kép** — lấy từ RTM (câu trả lời N17-*); không có trong RTM → **chèn ngay một lượt hỏi mở cho message đó** (20 §3b: message nguyên văn là vùng duy nhất không có mặc định ngành, và giờ hỏi được); hết giờ hỏi thì viết message cụ thể và mở dòng `G-xx` rủi ro **Cao**, KHÔNG ghi "hiển thị thông báo lỗi phù hợp". Có cả rule nghiệp vụ, không chỉ rule format.
5. **Mục 8 Xác thực & phân quyền** (450 token): bảng actor bắt buộc có **Guest** và **System/Job**; ô không ✓ = ✗ theo 0.4. Vì 0.4 biến mọi im lặng thành CẤM, bảng phải phủ cả thao tác chỉ-đọc (xem hold của mình / mọi hold, xem audit log) và mọi hành động log nêu "được làm". Bảng 2: mỗi actor xác thực bằng gì.
6. **Mục 2 Item màn hình** (850 token): bảng `No · 項目名 · コントロール · I/O · 必須 · 備考`, chia theo khu vực. Bắt buộc có **khác biệt login vs guest**, điều kiện **hiển thị/ẩn/disable**, giá trị mặc định, giới hạn, format, text nút nguyên văn. Ô không có dữ kiện RTM → `[GIẢ ĐỊNH — ..]`, không để trống.
7. **Mục 3 Event** (500 token — đủ chỗ cho cả event hệ thống: job hết hạn, webhook kho, callback cổng, không chỉ event người bấm): bảng `No · Event · Trigger · Xử lý · Ghi chú` — tối thiểu: mở màn hình lấy gì từ đâu, mỗi nút gọi xử lý gì, double-click (chặn theo 0.10), đổi số lượng/dropdown cập nhật gì.
8. **Mục 7** (900 token — 7.2 nhắm **8–10 ca**, không dừng ở 4 ca tối thiểu): 7.1 ràng buộc + giảm nhẹ; **7.2 ca bất thường liên logic — bắt buộc đủ 4 ca BTC nêu đích danh**: dữ liệu đổi giữa lúc hiển thị và lúc submit · gửi trùng · mở link hai lần · mail fail sau khi đã lưu; cộng ca miền giữ hàng (hai khách tranh SKU cuối, kho điều chỉnh khi đang hold, cọc thất bại, tài khoản bị khóa, SKU ngừng bán, hết hạn đúng lúc chuyển đơn); 7.3 lỗi hệ thống chung; **7.4 một bảng riêng gom điều chưa chốt** kèm giả định hiện tại.
9. **Mục 9** (550 token: sơ đồ hệ thống **Mermaid `flowchart LR`** — **BTC yêu cầu tường minh** — knowledge/33 §7.4, + bảng timing + **bảng API** `API · Trigger · Payload · Response OK · Response lỗi → làm gì`), **Mục 10** (350 token: field/perf/security/audit, ghi rõ số nào là giả định; `erDiagram` §7.6 nếu quan hệ dữ liệu không hiển nhiên), **Mục 5** (350 token: **Mermaid `block-beta`** theo knowledge/33 §7.1 + 1–2 câu mô tả bố cục + **bảng ba trạng thái** mặc định/lỗi/thành công với cột Hiện gì · Nút chính · Message). **Bản nộp không có ảnh** — không dùng link Figma dù BTC nêu hình thức đó. Không bỏ trống mục nào.
...
10. Ghi `spec.md`, rồi **sinh ngay bản nộp** `spec.nop.md` (bước 13) và **đếm token trên BẢN NỘP**, không trên `spec.md` (bản nội bộ dài hơn ~500 token vì mang `← A-xx`, nhãn `[M-x]`, `(verify …)`):
    ...
    - **> 9.000** → sang chế độ nén rồi quay lại bước 11. Thứ tự hy sinh: **BR nhãn `[M-0]`** → câu nói về chính spec (40 §2 nhóm 22) → cắt theo knowledge/33 §6 (**mục 10 → 9 → 5 → 3 → 2**), và gộp BR trùng nội dung về đúng mục của nó. **Không cắt mục 1, 6, 7, 4, 8.** **Không cắt** 0.11–0.15 và không cắt luật cho ca M6 có tiền dính vào.
    - **< 7.500** → **chưa xong, không phải gọn.** Tiêu đệm theo bảng ưu tiên knowledge/33 §5: thêm case lỗi/biên cho logic có ít hơn 5 case → thêm message nguyên văn cho mọi nhánh từ chối chưa có → nâng 7.2 lên 8–10 ca → vẽ lưu đồ `flowchart TD` → bảng ba trạng thái mục 5 → `erDiagram` mục 10. In rõ đã tiêu vào đâu và token trước/sau.
...
14. In bảng tóm tắt 6 khối:
    - token (nội bộ / bản nộp, cả hai công thức) + đệm còn lại so với 10.000 **và khoảng cách tới sàn 7.500** | 10 mục có/thiếu + token thực tế từng mục so với ngân sách knowledge/33 §5 | số BR/EX | số logic mục 6 và **số case mỗi logic (logic nào < 5 case)** | số sơ đồ mermaid (phải ≥ 3) | dòng RTM đã phủ / tổng (tách `A-xx` và `G-xx`)
...
## Output bắt buộc
- [ ] `spec.md` đủ **10 mục BTC**, không mục nào trống, mọi ô bảng có giá trị; bản nộp **≤9.000 token (đích), tuyệt đối ≤10.000, và ≥7.500 hoặc đã chứng minh không còn ô trống**.
- [ ] Mục 6: mỗi logic có bảng step + bảng Case đủ 3 loại case, **≥5 case**, và 5 gạch; cột kết quả đủ 7 chiều.
...
- [ ] **Đủ ba sơ đồ BTC yêu cầu** (mục 5 wireframe · mục 6 sequence end-to-end · mục 9 sơ đồ hệ thống) + lưu đồ `flowchart TD` cho logic nhiều nhánh nhất; mỗi sơ đồ kèm 1–2 câu tóm tắt; đã chạy lint knowledge/33 §7.7.
...
- Nộp khi token bản nộp > 10.000.
- **Dừng ở dưới 7.500 token trong khi còn mục có ô trống, logic có < 5 case, hoặc nhánh từ chối chưa có message.** Với hạn mức 10.000, tiết kiệm token không phải là đức tính — chỗ trống là chỗ Executor đoán.
- Bỏ một trong ba sơ đồ BTC yêu cầu tường minh (mục 5, 6, 9) vì "không đủ chỗ" khi token còn dưới 9.000.
