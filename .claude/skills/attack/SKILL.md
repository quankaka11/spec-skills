...
## Input
`$ARGUMENTS` = `cheo <spec1> <spec2> <spec3> [thư-mục]` (chế độ đồng thuận chéo, **chạy trước tiên lúc 13:00**) hoặc `<spec-đối-thủ> <tên-đội> [thư-mục-trận]`. Thư mục trận mặc định `battle/` — lấy `rtm.md` (dòng `A-xx` ⚠ = băng đạn chắc nhất; dòng `G-xx` = đạn suy luận; mục NGOÀI phạm vi = rào chống VÔ HIỆU), `log-khach-hang.md` (5 câu trả lời nguyên văn + timestamp), `brief.md` (bằng chứng phạm vi mức 2), `review.md` mục `RỦI RO ĐÃ BIẾT` (khi tự bắn spec mình).

Đọc trước:
- `${CLAUDE_PROJECT_DIR}/knowledge/50-tan-cong.md`: §1 (**21 loại**, gồm #16–#20 nội dung và **#21 lệch đồng thuận chéo**), §2 (quy tắc viết tình huống + **§2-9 ba mức bằng chứng phạm vi** + §2-10 bỏ slot), §3 (phân bổ 5 test + trọng số nguồn đạn mới), §4 (quy trình 10 phút + cổng F rút gọn bằng grep), **§4b (bảng đồng thuận chéo 3 spec)**, §5 (probe **P1–P52**), §6 (rubric phạm vi + **luật 8 điểm kỳ vọng**), §7 (hồ sơ finding + kháng nghị chỉ ca VÔ HIỆU).
...
5b. **Nạp ứng viên từ `dong-thuan-cheo.md`**: mọi dòng thuộc danh sách (a) và (b) liên quan tới spec này ⇒ ứng viên #21. Với 5 câu hỏi buổi sáng, danh sách ⚠ ở bước 5 chỉ 8–20 dòng, nên bước này thường là nguồn ứng viên lớn nhất (50 §3).
...
9. **Chọn tối đa 5 + 2 dự phòng** theo phân bổ knowledge/50 §3: các loại # khác nhau, phủ ≥4 nhóm N, tối đa 2 test cùng chủ đề. Ưu tiên: #15 → **#16/#18/#19/#21** → #2/#12 → #4/#11 → #7/#8/#17. Slot #9/#10 (actor, quyền) xuống dự phòng.
...
4. Dòng kiểm: các loại # khác nhau ✓ | mọi điểm phạm vi ≤1,5 ✓ | mọi test ≤60 từ ✓ | **mọi test có gói bằng chứng phạm vi mức 1/2/3 đã trích nguyên văn** ✓ | mọi test có `EV > 0` ✓ | **≥1 test thuộc #16–#21** ✓ | số test nộp = k (nêu rõ nếu k < 5).
